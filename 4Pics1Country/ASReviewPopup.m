...
-(void)showAlert {
	...
	if (!self.alertTitle)
		self.alertTitle = @"Are you having fun?";
    ...
    self.alertMessage = @"Rate this game now & get 25 bonus points!";
	
	if (!self.alertReviewButton)
		self.alertReviewButton = @"SURE!";
	
	if (!self.alertCancelButton)
		self.alertCancelButton = @"no";
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (kASReviewPopupAlertTag == [alertView tag]) {
		...
		if (buttonIndex == [alertView cancelButtonIndex]) {
			...
			if (self.appStoreURL) {
                NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                [defaults setBool:YES forKey:@"userLeftReview"];
                Jokers *bank = [[Jokers alloc] init];
                [bank LoadCount];
                [bank Add:25];
